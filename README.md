...
- **Game speed**: change the moves per second  
  ```js
  // snake.js
  export const SNAKE_SPEED = 5; // try 8 or 10 for faster
  ```

- **Grid size**: change the play area (also update CSS grid rows/cols in `index.html`)  
  ```js
  // grid.js
  const GRID_SIZE = 21; // try 25 or 30
  ```

- **Growth amount**: segments added when eating food  
  ```js
  // food.js
  const EXPANSION_RATE = 5; // try 1 or 3 for smaller growth
  ```

- **Colors**: change `.snake` and `.food` styles in `index.html`.

---

## 🧪 Troubleshooting

- **Blank page / modules not loading**: Serve via a **local server** (see Getting Started).  
- **Arrow keys do nothing**: Click the page once to focus the window; check your keyboard locale.  
- **Snake not growing**: Confirm `EXPANSION_RATE` > 0 and food is being redrawn.  
- **Instant 180° turn allowed**: Ensure you’re using `lastInputDirection` logic in `input.js` and not mutating it elsewhere.
...
## 👨‍💻 Author
Built with ❤️ as a learning project. Contributions, suggestions, and stars are welcome!
